...
def dt_gnc(data_name, dataset, model, insts_seed_file, feats_file, tmp_dir):
    axps = []
    cnf_nv = []
    cnf_claus = []
    T_time = 0
    ...
    for idx, line in enumerate(inst_data.X):
        ...
        axp, nv_cnf, claus_cnf, failed = feat_mem.GnC(f_id)
        if failed:
            failed_count += 1
        elif axp:
            print('Answer Yes')
    ...
    with open('results/kr22/xpg_fm_gnc.txt', 'a') as f:
        f.write(exp_results)


def obdd_gnc(data_name, model, insts_file, feats_file, tmp_dir):
    axps = []
    cnf_nv = []
    cnf_claus = []
    T_time = 0
...
def sdd_gnc(sdd_file, vtree_file, circuit, insts_file, feats_file):
    name = circuit
    ...
    for i, s in enumerate(inst_lines):
        ...
        sat_axp, nv_cnf, claus_cnf, failed = feat_mem.GnC(lits, f_id)
        if failed:
            failed_count += 1
        elif sat_axp:
            print("Answer Yes")
    ...
    with open('results/kr22/sdd_fm_gnc.txt', 'a') as f:
        f.write(exp_results)
    ...
    if len(args) >= 2 and args[0] == '-bench':
        ...
        for item in name_list:
            ...
            if args[2] == '-dt':
                ...
                dt_gnc(name, dataset, in_model, test_insts_seed, test_feats, save_xpg)
            elif args[2] == '-obdd':
                ...
                if in_model.nn > 7500:
                    obdd_gnc(name, in_model, test_insts, test_feats, save_xpg)
            elif args[2] == '-sdd':
                # circuit
                ...
                test_insts = f"samples/test_insts/{bench_name}/{name}.csv"
                test_feats = f"samples/test_feats/{bench_name}/{name}.csv"
                sdd_gnc(circuit_sdd, circuit_sdd_vtree, name, test_insts, test_feats)
    exit(0)
