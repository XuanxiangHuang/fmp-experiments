...
# -*- coding:utf-8 -*-
#
#   Reproduce experiments (simply answer fmp)
import csv
import resource
import sys
...
def dt_answer_fm(data_name, dataset, model, insts_seed_file, feats_file, tmp_dir):
    weakaxps = []
    cnf_nv = []
    cnf_claus = []
    T_time = 0
    ...
    exp_results += f"{nv} & {nn} & "
    exp_results += f"{ceil(len(weakaxps) / d_len * 100):.0f} ({ceil(failed_count / d_len * 100):.0f}) & "
    ...
    with open('results/kr22/xpg_fm_answer.txt', 'a') as f:
        f.write(exp_results)


def obdd_answer_fm(data_name, model, insts_file, feats_file, tmp_dir):
    weakaxps = []
    cnf_nv = []
    cnf_claus = []
    T_time = 0
...
def sdd_answer_fm(sdd_file, vtree_file, circuit, insts_file, feats_file):
    name = circuit
    ...
    for i, s in enumerate(inst_lines):
        ...
        if failed:
            ...
            print("Answer Yes")
            sat_weakaxps.append(sat_weakaxp)
        else:
            print('=============== no AXp exists ===============')
        cnf_nv.append(nv_cnf)
        cnf_claus.append(claus_cnf)
    ...
    exp_results += f"{nv} & nn & "
    exp_results += f"{ceil(len(sat_weakaxps) / d_len * 100):.0f} ({ceil(failed_count / d_len * 100):.0f}) & "
    exp_results += "{0:.0f} & {1:.0f} & " \
        .format(sum(cnf_nv) / d_len, sum(cnf_claus) / d_len)
    exp_results += "{0:.1f} & {1:.1f} & {2:.1f} & {3:.1f}\n" \
        .format(T_time, M_time, m_time, T_time / d_len)
    ...
    with open('results/kr22/sdd_fm_answer.txt', 'a') as f:
        f.write(exp_results)
    ...
    if len(args) >= 2 and args[0] == '-bench':
        ...
        for item in name_list:
            ...
            if args[2] == '-dt':
                ...
                dt_answer_fm(name, dataset, in_model, test_insts_seed, test_feats, save_xpg)
            elif args[2] == '-obdd':
                ...
                if in_model.nn > 7500:
                    obdd_answer_fm(name, in_model, test_insts, test_feats, save_xpg)
            elif args[2] == '-sdd':
                # circuit
                ...
                test_insts = f"samples/test_insts/{bench_name}/{name}.csv"
                test_feats = f"samples/test_feats/{bench_name}/{name}.csv"
                sdd_answer_fm(circuit_sdd, circuit_sdd_vtree, name, test_insts, test_feats)
    exit(0)
